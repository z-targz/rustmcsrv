...
static INHERITABLES: LazyLock<Mutex<
    HashMap<StructName, HashMap<FieldName, (FieldType, FieldAttrs)>>
            ...
            Some(path) => {
                if lock.contains_key(&path.to_string()) {
                    ...
                    fields_to_inherit.extend(
                        lock.get(&path.to_string()).unwrap()
                            .iter()
                    ...
                    match meta.value()?.parse::<syn::Expr>()? {
                        syn::Expr::Array(arr) => {
                            let the_vec = arr.elems.into_iter().map(|elem| {
                                match elem {
                                    syn::Expr::Lit(lit) => {
                                        match lit.lit {
                                            syn::Lit::Str(lit_str) => {
                        ...
                        _ => Err(meta.error("must be a single string literal or slice of string literals")),
                    }
                } else {
        ...
        syn::Data::Struct(ref mut data_struct) => {
            match &mut data_struct.fields {
                syn::Fields::Named(ref mut fields) => {
                    ...
                    fields_to_inherit.into_iter().map(|(field_name, (field_type, field_attrs))| {
                ...
                _ => Err(syn::Error::new(data_struct.fields.span(), "Fields are not named."))
            }
        },
        _ => Err(syn::Error::new(ast.span(), "This macro can only be used with structs."))
    };
                    ...
                    data_struct.fields.iter().map(|f| {
                        (
                            ...
                            (
                                ...
                                f.attrs.iter().map(|a| a.to_token_stream().to_string()).collect::<Vec<_>>(), 
                            )
                        )
